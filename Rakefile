IGNORE_FILES = ['Rakefile', 'README.markdown', '.gitignore', 'notes', 'files', 'config', 'atom', 'prep.sh', 'xapps.sh']
...
task :install do
  ...
  # Link dot files in ~/.atom directory.
  atom_folder = File.expand_path("~/.atom")
  unless File.exist?(atom_folder)
      Dir::mkdir(atom_folder)
  end
  Dir["atom/*"].each do |file|
      source = File.join(Dir.pwd, file)
      destination = "#{atom_folder}/#{File.basename(file)}"
      link_file(source, destination)
  end
  ...
  unless File.exist?(composer_file)
      ...
      info_cmd "Composer exists, updating"
      `php $HOME/.bin/composer.phar selfupdate`
  end
